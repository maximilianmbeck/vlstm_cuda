...
void kernel_dispatchers::mmkernelv1_dispatch(scalar_t *matC, scalar_t *matA,
  ...
  const dim3 gridDims((m + blockDims.y - 1) / blockDims.y,
...
*/
template <typename scalar_t>
void kernel_dispatchers::mmkernelv2_dispatch(scalar_t *matC, scalar_t *matA,
                                             scalar_t *matB, int m, int n,
                                             int k) {
  printf("m: %d, n: %d, k: %d\n", m, n, k);
  const int BLOCK_SIZE = 4;
  ...
  //     return;
  // }
  kernels::mmkernelv2<scalar_t, BLOCK_SIZE>
      <<<gridDims, blockDims>>>(matC, matA, matB, m, n, k);
...
template void kernel_dispatchers::mmkernelv2_dispatch<__nv_bfloat16>(
    __nv_bfloat16 *matC, __nv_bfloat16 *matA, __nv_bfloat16 *matB, int m, int n,
    int k);
template void kernel_dispatchers::mmkernelv2_dispatch<__half>(
    __half *matC, __half *matA, __half *matB, int m, int n, int k);
