...
for HEAD_DIM in HEAD_DIMS:
    configs.append(
        triton.testing.Benchmark(
            ...
            line_arg="provider",
            line_vals=[
                "mlstm_pt_ag_compile_fwbw",
                "mlstm_triton_fwbw",
                "mlstm_pt_ag_compile_fw",
                "mlstm_triton_fw",
            ],
            line_names=[
                "mLSTM PT Autograd Compile FWBW",
                "mLSTM Triton FWBW",
                "mLSTM PT Autograd Compile FW",
                "mLSTM Triton FW",
            ],  # ["mLSTM PT", "mLSTM PT Compile", "mLSTM Triton"],
            styles=[("red", "-"), ("blue", "-"), ("red", "--"), ("blue", "--")],
            ylabel="ms",
            plot_name=f"mlstm_parallel_fwbw_fw-batch{BATCH}-head{N_HEADS}-d{HEAD_DIM}",
            args={
                "H": N_HEADS,
                "BATCH": BATCH,
                "HEAD_DIM": HEAD_DIM,
...
def bench_flash_mlstm_fwbw(BATCH, H, N_CTX, HEAD_DIM, provider, device="cuda"):
    ...
    if "triton" in provider:
        fw_fn = lambda: mlstm_triton(q, k, v, ig, fg)
