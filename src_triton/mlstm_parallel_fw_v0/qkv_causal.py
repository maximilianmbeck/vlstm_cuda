...
def _qkv_fwd(
    ...
    for start_n in range(lo, hi, BLOCK_KV):
        ...
        # causal masking
        #! TODO get a weird error: operation scheduled before its operands
        if start_n >= hi_causal:
            # we are on diagonal
            kv_block_idxes = start_n + tl.arange(0, BLOCK_KV)
            mask = q_block_idxes[:, None] - kv_block_idxes[None, :]
            qk = tl.where(mask >= 0, qk, tl.zeros_like(qk))
