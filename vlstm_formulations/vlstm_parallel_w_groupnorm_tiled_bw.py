...
def vlstm_parallel_w_groupnorm_torch_bw(
    ...
    # FGATE VARIANT 2: EFFICIENT #! not working yet
    # # more efficient way would be # TODO sure? does not yield same numerical results
    # delta_fbar = delta_Dtilde.cumsum(-1).tril(-1).sum(dim=-2)

    # delta_f = (delta_fbar * torch.sigmoid(-vecF.squeeze(-1))).unsqueeze(-1)
    #! DEBUG only
    # delta_f = delta_fbar
